...
IndexedDB is the source of truth; localStorage is not used for transactional data. Dexie schema version 7 is lazy-opened in the browser. React depends on repository contracts rather than tables.

Current tables retain v1–v6 and add cashForecasts, supplierReviews, and projectCloseouts. Earlier accounting and operational tables remain intact. Company-owned records contain `companyId`, UUIDs, timestamps, and human codes. First-run creation is atomic. Drafts survive refresh. Backup v7 includes all current tables; legacy backups remain restorable with newer tables empty.
