...
export interface SubcontractCertificate extends BaseEntity { number:string;projectId:string;subcontractorId:string;date:string;grossAmount:number;retentionAmount:number;advanceRecovery:number;netDue:number;paidAmount:number;status:BillingStatus;journalId?:string }
