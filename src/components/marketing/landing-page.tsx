...
import { ArrowLeft, BarChart3, Boxes, Building2, CheckCircle2, ChevronLeft, Menu, ShieldCheck, TrendingUp, WalletCards, X } from 'lucide-react';
...
const nav = [['الرئيسية','#home'],['المميزات','#features'],['الدورة التشغيلية','#workflow'],['المحاسبة','#accounting'],['المشروعات','#projects'],['التقارير','#dashboards'],['الأسئلة الشائعة','#faq']];
const pills = ['إدارة المشروعات','دورة محاسبية كاملة','مخازن ومشتريات','مقاولو باطن','مستخلصات','تقارير وربحية'];
...
export function LandingPage() {
  ...
  return (
    <main id="home" className="min-h-screen overflow-x-hidden bg-[#f7fafc] text-slate-950" dir="rtl">
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 lg:px-10">
          <Link href="#home" className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0b527d] text-lg font-black text-white shadow-lg shadow-blue-900/15">م</span><span><strong className="block text-base text-[#0c3156]">منظومة المقاولات</strong><small className="block text-[10px] tracking-wide text-slate-500">Industrial Contracting ERP</small></span></Link>
          ...
          <div className="hidden items-center gap-2 sm:flex"><Link href="/setup" className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-700">تسجيل الدخول</Link><Link href="/setup" className="rounded-xl bg-[#0c5f8f] px-5 py-2.5 text-xs font-extrabold text-white shadow-lg shadow-blue-900/10">{cta}</Link></div>
      ...
      <section className="landing-grid relative overflow-hidden pb-20 pt-16 lg:pb-28 lg:pt-24">
        ...
        <div className="relative mx-auto grid max-w-[1440px] items-center gap-14 px-5 lg:grid-cols-[.9fr_1.1fr] lg:px-10">
          <div>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-teal-200 bg-white/80 px-4 py-2 text-xs font-bold text-[#147d75] shadow-sm"><ShieldCheck size={15}/>مصمم خصيصًا للمقاولات الصناعية</div>
            <h1 className="max-w-3xl text-4xl font-black leading-[1.25] tracking-tight text-[#0b2947] sm:text-5xl lg:text-[62px]">كل مشروع تحت السيطرة<span className="block text-[#148a83]">من العقد وحتى آخر قيد محاسبي</span></h1>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-600 lg:text-lg">منظومة ERP متكاملة لشركات المقاولات الصناعية تربط المشروعات والمشتريات والمخازن ومقاولي الباطن والمستخلصات والخزينة والدورة المحاسبية في مكان واحد.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/setup" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#0c5f8f] px-7 py-4 text-sm font-extrabold text-white shadow-[0_18px_40px_rgba(12,95,143,.22)] transition hover:-translate-y-0.5">{cta}<ArrowLeft size={18}/></Link><Link href="#workflow" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-7 py-4 text-sm font-extrabold text-slate-700 shadow-sm">اكتشف دورة العمل<ChevronLeft size={18}/></Link></div>
